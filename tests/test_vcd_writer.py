...
class TestGenerateMergedVcd:
    ...
    def test_basic_vcd(self, sample_waveform_data):
        ...
        item1 = WaveformItem(_waveform=wf1)
        item2 = WaveformItem(_waveform=wf2)

        with tempfile.NamedTemporaryFile(suffix='.vcd', delete=False) as f:
            path = f.name

        try:
            result = generate_merged_vcd([item1, item2], output_path=path)
            assert result == path
    ...
    def test_vcd_scope_hierarchy(self, sample_waveform_data):
        ...
        try:
            generate_merged_vcd([item], output_path=path)
    ...
    def test_custom_timescale(self, sample_waveform_data):
        ...
        try:
            generate_merged_vcd([item], output_path=path, timescale="100ps")
